...
function bboxToTiles(bbox: BBox, zoom: number): Tile[] {
    ...
    const yMin = Math.floor(
        ((1 - Math.log(Math.tan((north * Math.PI) / 180) + 1 / Math.cos((north * Math.PI) / 180)) / Math.PI) / 2) * numTiles
    );
    const yMax = Math.floor(
        ((1 - Math.log(Math.tan((south * Math.PI) / 180) + 1 / Math.cos((south * Math.PI) / 180)) / Math.PI) / 2) * numTiles
    );
...
async function fetchTiles(
    ...
    const tilePromises = allTiles.map(async ({ x, y, z }) => {
        ...
        try{
            ...
            console.error(`Error fetching contour tile for z:${z},x:${x},y:${y}`, error)
